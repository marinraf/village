...
class Settings():
    def __init__(self, main_settings, duration_settings, directory_settings, 
                    alarm_settings, telegram_settings, advanced_settings,
                    touchscreen_settings, screen_settings, sound_settings, 
                    bpod_settings, bpod_advanced_settings, 
                    harp_settings, harp_advanced_settings, extra_settings):
        ...
        self.restorable_settings = main_settings + duration_settings + touchscreen_settings + \
            screen_settings + sound_settings + alarm_settings + \
            bpod_settings + bpod_advanced_settings + harp_settings + harp_advanced_settings + \
            directory_settings + advanced_settings
       
        self.all_settings = self.restorable_settings + extra_settings + telegram_settings
...
main_settings = [
    ...
    Setting('CONTROL_DEVICE', 'Bpod', ControlDevice, 'The device that controls the tasks and behavioral ports')
]

duration_settings = [
    Setting('DEFAULT_REFRACTARY_PERIOD', 14400, int, 
            'Period of time in seconds that the animal is not allowed to enter after a completed session from the same animal'),
    Setting('DEFAULT_DURATION_MIN', 1800, int, 
            'Default minimum duration of the session in seconds. Door2 is opened after this time.'),
    Setting('DEFAULT_DURATION_MAX', 3600, int, 
            'Default maximum duration of the session in seconds. The session is ended after this time.')
]

directory_settings = [
    Setting('APP_DIRECTORY', '/home/mousevillage/mouse_village', str, 'The directory of the application'),
    Setting('USER_DIRECTORY', '/home/mousevillage/user', str, 'The directory of the user'),
    Setting('DATA_DIRECTORY', '/home/mousevillage/data', str, 'The directory of the data'),
    Setting('BACKUP_TASKS_DIRECTORY', '/home/mousevillage/backup_tasks', str, 
            'Directory where a copy of the task with a particular code is saved every time a task is run')
]

alarm_settings = [
    Setting('MINIMUM_WATER_24', 400, int, 'Minimum water in ml for 24 hours. If the animal drinks less, an alarm is triggered'),
    Setting('MINIMUM_WATER_48', 1000, int, 'Minimum water in ml for 48 hours. If the animal drinks less, an alarm is triggered'),
    Setting('MINIMUM_TEMPERATURE', 19, int, 'Minimum temperature in celsius. If the temperature is lower, an alarm is triggered'),
    Setting('MAXIMUM_TEMPERATURE', 27, int, 'Maximum temperature in celsius. If the temperature is higher, an alarm is triggered')
]

telegram_settings = [
    Setting('TOKEN', '', str, 'The token of the telegram bot'),
    Setting('TELEGRAM_CHAT', '', str, 'The chat id of the telegram bot'),
    Setting('TELEGRAM_USERS', ['', '', '', '', ''], list, 'The users allowed to use the telegram bot')
]

advanced_settings = [
    Setting('TAG_DURATION', 0.5, float, 'The duration of the tag in seconds'),
    Setting('DIFFERENT_TAG_SEPARATION', 15.0, float, 
            'If a tag is detected but the previous one was less than this time ago, animal is not allowed to enter'),
    Setting('CAM_CORRIDOR_DURATION_VIDEO', 1800, int, 'Duration of the corridor videos in seconds.'),
    Setting('CAM_CORRIDOR_VIDEOS_STORED', 100, int, 'The number of corridor videos stored')
]

touchscreen_settings = [
    Setting('TOUCH_RESOLUTION', (4096, 4096), tuple, 'The resolution for the reading of the touch screen'),
    Setting('SCREEN_SIZE_MM', (400, 200), tuple, 'The size of the screen in mm'),
    Setting('TIME_BETWEEN_TOUCHES_S', 0.5, float, 
...
sound_settings = [
    Setting('PARAMETER', 1, int, 'The parameter of the sound')
]

bpod_settings = [
    Setting('BPOD_TARGET_FIRMWARE', 22, int, 
    ...
    Setting('BPOD_BNC_PORTS_ENABLED', ['No', 'No'], list, 'The enabled BNC ports of the Bpod'),
    Setting('BPOD_WIRED_PORTS_ENABLED', ['No', 'No'], list, 'The enabled wired ports of the Bpod'),
    Setting('BPOD_BEHAVIOR_PORTS_ENABLED', ['Yes', 'No', 'No', 'No', 'No', 'No', 'No', 'No'], list,
            'The enabled behavior ports of the Bpod'),
    Setting('BPOD_BEHAVIOR_PORTS_WATER', ['No', 'No', 'No', 'No', 'No', 'No', 'No', 'No'], list,
...
bpod_advanced_settings = [
    ...
    Setting('BPOD_NET_PORT', 36000, int, 'The network port of the Bpod (for receiving softcodes)'),
...
extra_settings = [
    ...
    Setting('SESSIONS_DIRECTORY', '/home/mousevillage/data/sessions', str, 'The directory of the sessions')
]

settings = Settings(main_settings, duration_settings, directory_settings, 
                    alarm_settings, telegram_settings, advanced_settings,
                    touchscreen_settings, screen_settings, sound_settings, 
                    bpod_settings, bpod_advanced_settings, 
                    harp_settings, harp_advanced_settings, extra_settings)
